...
export default function EditKelas() {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        e.preventDefault()
        return await client.put(`/kelas/${idKelas}`, formData).then(res => {
            Swal.fire({
                title: 'Sukses',
                text: 'Berhasil edit kelas',
                icon: 'success'
            })
        }).catch(err => {
            console.log(err)
            ...
            return await client.get(`/kelas/${idKelas}`).then(res => {
                setFormData(prev => {
                    return {
                        ...
                        jurusanId: res.data.data.jurusanId,
                        tahunAjaranId: res.data.data.tahunAjaranId
                    }
                })
            }).catch(err => {
                console.log(err)
            ...
            <div className="bg-white p-5 shadow rounded-md">
                <form onSubmit={handleSubmit}>
                    <div className="flex items-center justify-between">
                        <h1 className="text-xl">Edit Kelas</h1>
